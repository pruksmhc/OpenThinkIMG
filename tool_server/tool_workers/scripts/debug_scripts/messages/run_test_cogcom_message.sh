...
=======
OMP_NUM_THREADS=8 srun --partition=MoE --job-name="test-ocr" --mpi=pmi2 --gres=gpu:${gpus}  -w SH-IDCA1404-10-140-54-119  -n1 --ntasks-per-node=1 -c ${cpus} --kill-on-bad-exit=1 --quotatype=${quotatype}  \
python test_ocr_messages.py --send_image \
>>>>>>> main
